...
export function addButtons(objects, id, pinned, planets) {
  ...
  // function to add buttons to collapsible
  export function addMissions(objects) {

    var buttonsContainer = document.getElementById('mission_library');
    i = 0;
    objects.forEach(function(objects) {
      var button = document.createElement("button");
      button.innerHTML = objects.replace(" BARYCENTER","");
      ...
      pinCheckbox.addEventListener("click", function() {
        ...
        if (this.checked){
          ...
          document.getElementById('pinned_missions').appendChild(button);
          document.getElementById('pinned_missions').appendChild(content);
        }else{
          // send it to the library list 
          parent_content.removeChild(content);
          parent.removeChild(button);

          document.getElementById('mission_library').appendChild(button);
          document.getElementById('mission_library').appendChild(content);
        }
      });
      var label = document.createElement("label");
      label.htmlFor = "pin_checkbox";
